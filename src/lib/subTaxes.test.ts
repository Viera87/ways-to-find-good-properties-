...
describe("subsequent tax golden rule", () => {
  ...
  it("always reserves at least the next full bill", () => {
    const shortHold = subsequentTaxReserve(200000, {
      subsequentTaxRate: 0.011,
      holdMonths: 5,
      subTaxMonth: 6,
    });
    assert.equal(shortHold, 2200);
    const longHold = subsequentTaxReserve(200000, {
      subsequentTaxRate: 0.011,
      holdMonths: 18,
      subTaxMonth: 6,
    });
    assert.equal(longHold, 4400);
  });

  it("fails the golden rule when desk cash covers ACH but not the next levy", () => {
    const plan = subsequentTaxPlan(
      { amountDue: 3000, assessedValue: 200000 },
      DEFAULT_ASSUMPTIONS,
      3000,
    );
    assert.equal(plan.reserve, 2200);
    assert.equal(plan.capitalToOwn, 5200);
    const fail = goldenRule(4000, plan.auctionDayCapital, plan.reserve);
    assert.equal(fail.pass, false);
    assert.equal(fail.shortfall, 1200);
    const ok = goldenRule(5200, plan.auctionDayCapital, plan.reserve);
